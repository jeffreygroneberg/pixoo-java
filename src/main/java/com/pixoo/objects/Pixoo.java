...
public class Pixoo {
    ...
    public void drawImage(BufferedImage image, MathUtils.Point xy, ImageResampleMode resampleMode, boolean padResample) {
        ...
        if (width > size || height > size) {
            Scalr.Method scalingMethod = (resampleMode == ImageResampleMode.PIXEL_ART) ? 
                Scalr.Method.SPEED : Scalr.Method.QUALITY;
            
            if (padResample) {
                // Pad to maintain aspect ratio
                image = Scalr.pad(image, size, java.awt.Color.BLACK);
            } else {
                // Scale to fit
                image = Scalr.resize(image, scalingMethod, size, size);
            }
            
            if (debug) {
                System.out.printf("[.] Resized image to fit on screen: (%d, %d) -> (%d, %d)%n", 
